...
if [[ $param == [123] ]]; then
   ...
   if [[ $param == "1" && $# != 5 ]]; then
      echo """
	   Usage: ./scanPart.R 1 <.hap+.map-root> <chr#> <output> <in-vcf>
			...
			in-vcf: Same VCF file that was provided to the 'phasedvcf2rehh.sh' script
      """
	...
	awk '{print $1"_v3."$2}' ${3}Signals.txt | sed '1d' > ${3}.grepfile.txt
	zgrep -f ${3}.grepfile.txt $5 | awk '{print $3}' > ${3}.signals.snpsids.txt
	rm ${3}.grepfile.txt

   elif [[ $param == "2" && $# != 8 ]]; then
        echo """
             Usage: ./scanFull.R 2 <.hap+.map-root> <output> <#chr> <thresh> <threads> <maf> <in-vcf>
	...
	awk '$6<=0.05' ${3}iHSresult.txt >> ${3}Signals.txt
        awk '{print $1"_v3."$2}' ${3}Signals.txt | sed '1d' > ${3}.grepfile.txt
        zgrep -f ${3}.grepfile.txt $8 | awk '{print $3}' > ${3}.signals.snpsids.txt
	rm ${3}.grepfile.txt
